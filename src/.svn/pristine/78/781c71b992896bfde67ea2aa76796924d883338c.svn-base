...
class Game extends Model {
    ...
    public List<String> getVictors() {
        ...
        if (stillStanding.size() == 1) {
            ...
            for (int i = 1; i < roundEvaluation.length; i++) {
                if (roundEvaluation[i] > largest) {
                    ...
                    victorPlayers.add(players[i]);
                }
            }
            if (victorPlayers.size() > 1) {
                for (int i = 1; i < victorPlayers.size(); i++) {
                    if (victorPlayers.get(i).getSilver() > victor.getSilver()) {
                        victor = victorPlayers.get(i);
                        victorPlayers.clear();
                        victorPlayers.add(victor);
                    } else if (victorPlayers.get(i).getSilver() == victor.getSilver()) {
        ...
        return playerListToStringList(victorPlayers);

    }
    
    /**
     * Returns a list with only the names of a given set of players.
     * 
     * @param players
     * @return 
     */
    private List<String> playerListToStringList(List<Player> players) {
