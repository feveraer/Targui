...
//Use Case 3 Begin  
        ...
        do
        {
            ...
            if(next.toLowerCase().charAt(0) == 'y'|| next.isEmpty())
            {
                ...
                else
                {
                    ...
                    while(!check[0])
                    {
                        coords = uc5.selectSquare(dc, check, input, turn);
                    }
                    
                    
                    int[][] adjecent = dc.getAdjacentSquares(coords[0], coords[1]);
    ...
    public int[] selectSquare(DomainController dc, boolean[] check, Scanner input, String[] turn)
    {
        int[] coords = new int[2];
        try {
    ...
    public void startBuyCamels(DomainController dc, Scanner input, UseCase2 uc2, String[] turn, boolean[] check)
    {
        try {
            ...
            if (exp.length == 1) {
                ...
                System.err.println(String.format(bundle.getString(exp[0]), exp[1], exp[2]));
            }
        }
    }
